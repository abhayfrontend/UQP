...
@Component({
  selector: 'app-overall-gaps',
  templateUrl: './overall-gaps.component.html',
  styleUrls: ['./overall-gaps.component.scss']
})
export class OverallGapsComponent implements OnInit {
  search_category: string = '';
  // global variables to identify user role entered/logged in
  rolename: string;
  roleid: any;
  // To check acl add/edit/view
  userRolePerm_suppl:any;
...
current_month = this.authS.getDates().current_month;
  ...
  constructor(public authS: AuthService, private router: Router,
    ...
    this.userRolePerm_suppl = this.authS.getPermission('Supplemental File Download');
    this.userRolePerm_down = this.authS.getPermission('Download Documents');
    console.log(this.userRolePerm_suppl)
    if(route.snapshot.data['title'] == "compliant"){
      this.compliant_report = true;
    };
  }
...
if(len > 150){
  ...
  openGapModal(member) {
    ...
    $('.gap-form').removeClass('restrict_pointer restrict_cursor');
    ...
    if(member.Isgapssubmit != 'notsubmitted'){
      $('.gap-form').addClass('restrict_pointer restrict_cursor')
    }
    ...
    if (member.Isgapssubmit == 'notsubmitted' || member.Isgapssubmit == 'rejected') {
      ...
      this.gapSubmit.dateofdexaneeded = member.dateofdexaneeded;
      this.gapSubmit.dateofdischarge = member.dateofdischarge;
      this.gapSubmit.dateofmedicalreconcilation = member.dateofmedicalreconcilation;
      this.setMemberDetails(member);
      this.childGapTemplate.checkDatesFromDB(this.gapSubmit);
      ...
      this.commonService.getGapDetail(this.gapSubmit).subscribe(results => {
        ...
        console.log(this.gapSubmit.dateofservice.date)
        this.setMemberDetails(member);
        this.childGapTemplate.checkDatesFromDB(this.gapSubmit);
        this.measureName = member.measname;
        this.modal.show();
 ...
 search_value(term)
  ...
  getAllMembers(resetPage) {
    ...
    this.commonService.getOverallGaps(this.measureArray(), this.params).subscribe(results => {
      ...
      this.member_list = results.body.map((member) => {
        // debugger;
        if(member.ReviewdBy){
          if(member.ReviewdBy.split(',').includes(this.currentUser.userid.toString())){
            member.Isreviewed = true;
          }else{
            member.Isreviewed = false;
          }
        }
        //check role 
        console.log(this.currentUser.roleid);
        let passIds = [4,5,35,36]
        if(passIds.includes(this.currentUser.roleid)){
          member.ReviewdBy = null;
        }        
        return member
      });
...
getAllHealthplans() {
    this.commonService.getAllHealthplans().subscribe(results => {
      ...
      this.insurance_list_all = results;
